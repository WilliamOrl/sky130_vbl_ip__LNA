v {xschem version=3.4.5 file_version=1.2
}
G {}
K {}
V {}
S {}
E {}
N 630 -360 820 -360 {
lab=GND}
N 550 -240 550 -190 {
lab=g}
N 550 -190 580 -190 {
lab=g}
N 710 -260 710 -190 {
lab=b}
N 620 -190 710 -190 {
lab=b}
N 620 -160 620 -120 {
lab=d}
N 620 -120 780 -120 {
lab=d}
N 780 -250 780 -120 {
lab=d}
N 780 -260 780 -250 {
lab=d}
N 870 -260 870 -220 {
lab=n}
N 820 -360 870 -360 {
lab=GND}
N 870 -360 870 -330 {
lab=GND}
N 870 -330 870 -320 {
lab=GND}
N 780 -360 780 -320 {
lab=GND}
N 710 -360 710 -320 {
lab=GND}
N 620 -360 620 -220 {
lab=GND}
N 620 -360 630 -360 {
lab=GND}
N 550 -360 550 -300 {
lab=GND}
N 550 -360 620 -360 {
lab=GND}
N 550 -380 550 -360 {
lab=GND}
C {devices/vsource.sym} 550 -270 0 1 {name=vg value="DC 2.5 AC 1" savecurrent=false}
C {devices/vsource.sym} 780 -290 0 0 {name=vd value=2.5 savecurrent=false}
C {devices/gnd.sym} 550 -380 2 0 {name=l2 lab=GND}
C {devices/vsource.sym} 710 -290 0 1 {name=vb value=0 savecurrent=false}
C {devices/lab_wire.sym} 550 -190 0 0 {name=p1 sig_type=std_logic lab=g}
C {devices/lab_wire.sym} 700 -120 0 0 {name=p2 sig_type=std_logic lab=d}
C {devices/lab_wire.sym} 710 -190 0 1 {name=p3 sig_type=std_logic lab=b}
C {devices/code_shown.sym} 0 -880 0 0 {name=COMMANDS1 only_toplevel=false
value="
.param wx=5 lx=0.5
.noise v(n) vg lin 100 0.1 100k 100

.control
option numdgt = 3
set wr_singlescale
set wr_vecnames

compose l_vec  values 0.5 0.51 0.52 0.53 0.54
+ 0.55 0.6 0.7 0.8 0.9 1 2 3
compose vg_vec start= 0 stop=5  step=50m
compose vd_vec start= 0 stop=5  step=50m
compose vb_vec start= 0 stop=-0.4 step=-0.2

foreach var1 $&l_vec
  alterparam lx=$var1
  reset
  foreach var2 $&vg_vec
    alter vg $var2
    foreach var3 $&vd_vec
      alter vd $var3
      foreach var4 $&vb_vec
        alter vb $var4
        run
        wrdata techsweep_pfet_g5v0d10v5.txt noise1.all
        destroy all
        set appendwrite
        unset set wr_vecnames  
      end
    end 
  end
end
unset appendwrite

alterparam lx=0.5
reset
op
show
write techsweep_pfet_g5v0d10v5.raw
.endc
"}
C {sky130_fd_pr/corner.sym} 1060 -750 0 0 {name=CORNER only_toplevel=true corner=tt}
C {devices/title.sym} 160 -40 0 0 {name=l5 author="Boris Murmann"}
C {devices/ngspice_get_value.sym} 1010 -290 0 0 {name=r1 node=v(@m.xm1.msky130_fd_pr__pfet_g5v0d10v5[vth])
descr="Vt="}
C {devices/launcher.sym} 940 -410 0 0 {name=h1
descr="load op & annotate" 
tclcommand="xschem raw_read $netlist_dir/techsweep_pfet_g5v0d10v5.raw; set show_hidden_texts 1; xschem annotate_op"}
C {devices/launcher.sym} 940 -450 0 0 {name=h3
descr="save, netlist & simulate"
tclcommand="xschem save; xschem netlist; xschem simulate"}
C {devices/ngspice_get_value.sym} 1010 -250 0 0 {name=r2 node=@m.xm1.msky130_fd_pr__pfet_g5v0d10v5[cgg]
descr="cgg="}
C {devices/ngspice_get_expr.sym} 1120 -210 0 0 {name=r4 
node="[format %.4g [expr [ngspice::get_node \{@m.xm1.msky130_fd_pr__pfet_g5v0d10v5[gm]\}] / [ngspice::get_node \{@m.xm1.msky130_fd_pr__pfet_g5v0d10v5[gds]\}]]]"
descr="gm/gds="}
C {devices/ngspice_get_value.sym} 1010 -210 0 0 {name=r3 node=@m.xm1.msky130_fd_pr__pfet_g5v0d10v5[capbd]
descr="capdb="}
C {devices/ngspice_get_value.sym} 1010 -170 0 0 {name=r5 node=@m.xm1.msky130_fd_pr__pfet_g5v0d10v5[capbs]
descr="capbs="}
C {devices/ngspice_get_expr.sym} 1120 -250 0 0 {name=r6 
node="[format %.4g [expr [ngspice::get_node \{@m.xm1.msky130_fd_pr__pfet_g5v0d10v5[gm]\}] / [ngspice::get_node \{@m.xm1.msky130_fd_pr__pfet_g5v0d10v5[cgg]\}] / 6.283]]"
descr="fT="}
C {devices/ngspice_get_expr.sym} 1120 -290 0 0 {name=r7 
node="[format %.4g [expr [ngspice::get_node \{@m.xm1.msky130_fd_pr__pfet_g5v0d10v5[gm]\}] / [ngspice::get_node \{i(@m.xm1.msky130_fd_pr__pfet_g5v0d10v5[id])\}]]]"
descr="gm/ID="}
C {devices/code_shown.sym} 530 -880 0 0 {name=COMMANDS2 only_toplevel=false
value="
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[capbd]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[capbs]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[cdd]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[cgb]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[cgd]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[cgg]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[cgs]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[css]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[gds] 
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[gm] 
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[gmbs] 
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[id]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[l]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[vbs]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[vds]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[vgs]
.save @m.xm1.msky130_fd_pr__pfet_g5v0d10v5[vth]
.save onoise.m.xm1.msky130_fd_pr__pfet_g5v0d10v5.id
.save onoise.m.xm1.msky130_fd_pr__pfet_g5v0d10v5.1overf
.save g d b n
"}
C {devices/ccvs.sym} 870 -290 0 0 {name=Hn vnam=vd value=1}
C {devices/lab_wire.sym} 870 -220 0 0 {name=p4 sig_type=std_logic lab=n}
C {sky130_fd_pr/pfet_g5v0d10v5.sym} 600 -190 0 0 {name=M1
L=\{lx\}
W=\{wx\}
nf=1 
mult=1
ad="'int((nf+1)/2) * W/nf * 0.29'" 
pd="'2*int((nf+1)/2) * (W/nf + 0.29)'"
as="'int((nf+2)/2) * W/nf * 0.29'" 
ps="'2*int((nf+2)/2) * (W/nf + 0.29)'"
nrd="'0.29 / W'" nrs="'0.29 / W'"
sa=0 sb=0 sd=0
model=pfet_g5v0d10v5
spiceprefix=X
}
